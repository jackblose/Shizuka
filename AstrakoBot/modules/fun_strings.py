RUN_STRINGS = (
    "LARIIIIII, DISINI BANYAK JAMET"
    "KABORRR,DISINI PADA BUCI",
    "KALIAN SEMUA ANJG HEHE",
    "BANYAK BANYAK BERLARI,JANGAN COLI",
    "KALIAN SEMUA STRESS,AKU KABUR AJA AH",
    "ORANG GILA SEMUA DISINI IH",
    "AKU LIAT KAU NGAMEN DI PERSIMPANGAN, AAAAAAAA",
    "AKU SAKIT PERUT MELIHAT KETOLOLAN MU",
    "LARI LEBIH BAIK DARI PADA COLI",
    "*ANU MU KECIL GAUSAH SOK KERAS,AWOWKWOWK*",
    "*LARI AH, SIAPA TAU DAPET TANTE-TANTE*",
    "OMEGAT!!!!!",
    "MUKA LU JELEK,MENDING GUA PARMIMG AH",
    "ANAK DAKJAL, AWKWKWKWKWK",
    "ANAK CAPER PADA NGAMOOKK, AWKKWWKWK CABUT AH ASU",
    "IDIHHHHH,GAK KEREN BEGO LU BEGITU AWOKWK",
    "KEREN LU BEGITU?, AAAAAA",
    "JELEK BAT GUA PUNYA TEMEN,GAYA NYA SOK KEREN PADAHAL SANGEAN!!",
    "NAJIS GUA, MUKA LU GUA INJAK²",
)

GIFS = [
    'CgACAgQAAx0CSVUvGgAC7KpfWxMrgGyQs-GUUJgt-TSO8cOIDgACaAgAAlZD0VHT3Zynpr5nGxsE',
    'CgACAgUAAx0CU_rCTAABAjdgX1s4NVaeCls6YaH3p43vgdCRwQIAAqsAA4P_MFUYQhyoR-kgpRsE',
    'CgACAgUAAx0CU_rCTAABAjdSX1s3fq5iEJ64YeQLKI8cD7CSuSEAAlUBAAJu09hW5iqWB0hTPD4bBA'
]

SLAP_SAITAMA_TEMPLATES = (
    "GAUSAH TAMPAR-TAMPAR GUA. GUA MUTE NIH",
    "STOP NAMPAR GUA. OOOOOASUUUUUU.",
    [
        "Saya bakal membisukan mu asu.",  # normal reply
        "Cukup!! Atau gua mute. Pantek!!.",  # reply to admin
        "tmute",  # command
        "Shut up!",
        "Silence!"
    ])

SLAP_TEMPLATES = (
    "{user1} MEMUKUL MU SAMPE MAMPUS",
    "{user1} MENTERTAWAKAN HIDUP MU YANG KUMUH",
    "{user2} DI PUKUL PALALO SAMA {user1}.",
    "{user2} MENINGGAL KEK LO!!",
    "{user2} MUKA LU DI GEPLAK SAMA {user1} SAMPE RATA.",
    "{user2} OTW DI BUNUH SAMA {user1} PAKE CARTER.",
    "{user1} MEMBUANG MU.",
    "{user2} PALALO DI LEMPAR ASBAK SAMA {user1}.",
    "{user2} MUKA LU BENER BENER HINA KATA {user1}.",
    "{user2} METODE MEMBUNUH TANPA MENYENTUH DI AKTIFKAN OLEH {user1}...",
    "{user2} PALA LO DI TENDANG SAMA {user1}.",
    "{user1} bilang dulu owner cakep biar gua geplak si {user2}.",
    "{user2} DIH ANJG PEN GUA TAMPOL LU.",
    "{user1} aku hah kamu ahhh yaaa sayang {user2}.",
    "{user1} MENGINJAK BATANG LEHER {user2} PAKE SENDAL JEPIT.",
    "{user1} MEMPERMALUKAN {user2} SAMPE DELETE TELE.",
    "{user1} KASIAN, {user2} DI TINGGAL PACARNYA SAMPE BUNUH DIRI.",
    "{user1} menampar {user2} dengan ilmu santet",
    "{user2} dibawa ke rumah sakit jiwa oleh {user1}.",
    "{user1} mencium dan meng w kak {user2}.",
    "{user2} Kata owner gabole kasar tapi lu kek anj hehe.",
    "{user1} menendang {user2} sampai terjatuh ke jurang.",
    "{user2} dipukul dengan satu pukulan oleh {user1}.",
    "{user1} menginjak injak kepala {user2} sampai pecah berkeping keping ",
    "{user2} pengen kasar tapi gua udah tobat anj.",
    "{user1} memeluk mu dengan erat karna sangat rindu kepada {user2}.",
    "{user1} melamar {user2} dengan mahar atm kosong.",
    "{user2} aku tau hidupmu berat, terimakasih sudah berjuang sampai semarang.",
    "{user2} Pacar Lu Udah Gua Grepe Anying {user1}.",
    "{user1} galauin apasi kawan, jadian aja ngga!!!.",
    "{user2} mengakui bahwa dirinya buriking hahaha.",
    "{user1} Menampar keras seperti tamparan suami psikopat.",
    "Sekarang adalah waktunya anda pergi dari sini {user2}.",
    "Heh {user2} Mau Lo Apansi Anjg.",
    "{user1} MEMBUNUH PSIKOPAT {user2}.",
)

PAT_TEMPLATES = (
    "{user1} memuji {user2} karna wajah yang indah menawan.",
    "*Menyukai  {user2} karna menyenangkan*.",
    "*{user1} memandang {user2} dengan pikiran mesum*",
    "*{user1} berkata dalam hati bahwa {user2} sangat menawan*",
    "*{user1} menginginkan  {user2} untuk menjadi pacarnya*",
    "*{user2} kamu memang pantas di nobatkan sebagai makhluk terindah*",
    "*{user2} kamu pantas mendapatkan gelar manusia ter keren*",
    "pakai dulu celanamu {user2}!",
    "Akuilah bahwa dirimu berguna",
    "pahami lah kamu itu hebat kata {user1}!",
    "bersyukurlah {user2} karna hidup mu adalah anugrah dari per*****tan ortumu!",
    "{user1} mencium {user2} dengan penuh hasrat!",
    "{user2} mendapatkan satu ke ajaiban dari tuhan!",
    "{user1} kepala mu di jitak {user2} sampe peyang",
    "{user1} sangat agresiv kepada {user2}",
    "{user1} memandangi  {user2} dari tadi, sange kali ya?",
    "Ngewe yuk",
    "{user2} sange berat sama {user1}",
    "{user2}, jangan kerja terus, skali kali ngopi",
    "{user1} mengewe {user2} sampai lelah",
    "Jangan pernah jadi orang goblok di grup ini",
    "{user1} memuji dengan penuh kesangean {user2}",
    "{user1} memeluk {user2} dengan erat",
    "{user1} mencolek {user2} dengan penuh hasrat, Sange ya?",
    "Pok pok, {user2} ngasih kode ngajak vcs",
    "{user1} mengelus pala {user2} yang katanya anak dakjal",
    "{user1} mencipok {user2} secara mendadak",
    "{user1} ngaceng, {user2} ngangkang.",
    "{user2} memberikan suatu hadiah kepala kuda liar.",
    "{user1} berkata. Jangan lah engkau menyia nyia kan hidup mu demi {user2} yang tidak sama skali memperhatikanmu.",
    "{user2} udah ngangkang tuh, {user1} gamau ngewe kah?",
    "{user1} udah ngaceng tuh, {user2} gamau goyang kah?",
)

PAT_GIFS = (
    "CAACAgUAAx0CX0AekgADfmDaMaM0xv53F8Vj6iUmUlS_bHElAALqAgACuGwxVt3wWX7qnIvDIAQ",
    "CAACAgUAAx0CX0AekgADg2DaMq0FjAKPHWFQWqhCu6qEoI5-AAJSAgAC7IchVh-IHEUtfQ5HIAQ")

PAT_STICKERS = (
    "CAACAgUAAx0CX0AekgADhmDaMwejUSqEvE_nAfYltFZGdB3PAALvAwACP38YVrXt-yHH68XiIAQ",
    "CAACAgUAAx0CX0AekgADiWDaM18aCW_fYqJ73yk2lHoFKjfRAAIGAgACZYPYVVj8ucG8v-e-IAQ")

PING_STRING = (
    "PONG!!",
    "I am here!",
)

ITEMS = (
    "cast iron skillet",
    "angry meow",
    "cricket bat",
    "wooden cane",
    "shovel",
    "toaster",
    "book",
    "laptop",
    "rubber chicken",
    "spiked bat",
    "heavy rock",
    "chunk of dirt",
    "ton of bricks",
    "rasengan",
    "spirit bomb",
    "100-Type Guanyin Bodhisattva",
    "rasenshuriken",
    "Murasame",
    "ban",
    "chunchunmaru",
    "Kubikiribōchō",
    "rasengan",
    "spherical flying kat",
)

THROW = (
    "throws",
    "flings",
    "chucks",
    "hurls",
)

HIT = (
    "hits",
    "whacks",
    "slaps",
    "smacks",
    "bashes",
    "pats",
)

EYES = [['⌐■', '■'], [' ͠°', ' °'], ['⇀', '↼'], ['´• ', ' •`'], ['´', '`'],
        ['`', '´'], ['ó', 'ò'], ['ò', 'ó'], ['⸌', '⸍'], ['>',
                                                         '<'], ['Ƹ̵̡', 'Ʒ'],
        ['ᗒ', 'ᗕ'], ['⟃', '⟄'], ['⪧', '⪦'], ['⪦', '⪧'], ['⪩', '⪨'], ['⪨', '⪩'],
        ['⪰', '⪯'], ['⫑', '⫒'], ['⨴', '⨵'], ['⩿', '⪀'], ['⩾', '⩽'], ['⩺', '⩹'],
        ['⩹', '⩺'], ['◥▶', '◀◤'], ['◍', '◎'], ['/͠-', '┐͡-\\'], ['⌣', '⌣”'],
        [' ͡⎚', ' ͡⎚'], ['≋'], ['૦ઁ'], ['  ͯ'], ['  ͌'], ['ළ'], ['◉'], ['☉'],
        ['・'], ['▰'], ['ᵔ'], [' ﾟ'], ['□'], ['☼'], ['*'], ['`'], ['⚆'], ['⊜'],
        ['>'], ['❍'], ['￣'], ['─'], ['✿'], ['•'], ['T'], ['^'], ['ⱺ'], ['@'],
        ['ȍ'], ['  '], ['  '], ['x'], ['-'], ['$'], ['Ȍ'], ['ʘ'], ['Ꝋ'],
        [''], ['⸟'], ['๏'], ['ⴲ'], ['◕'], ['◔'], ['✧'], ['■'], ['♥'], [' ͡°'],
        ['¬'], [' º '], ['⨶'], ['⨱'], ['⏓'], ['⏒'], ['⍜'], ['⍤'], ['ᚖ'], ['ᴗ'],
        ['ಠ'], ['σ'], ['☯']]

MOUTHS = [['v'], ['ᴥ'], ['ᗝ'], ['Ѡ'], ['ᗜ'], ['Ꮂ'], ['ᨓ'], ['ᨎ'],
          ['ヮ'], ['╭͜ʖ╮'], [' ͟ل͜'], [' ͜ʖ'], [' ͟ʖ'], [' ʖ̯'], ['ω'], [' ³'],
          [' ε '], ['﹏'], ['□'], ['ل͜'], ['‿'], ['╭╮'], ['‿‿'], ['▾'], ['‸'],
          ['Д'], ['∀'], ['!'], ['人'], ['.'], ['ロ'], ['_'], ['෴'], ['ѽ'], ['ഌ'],
          ['⏠'], ['⏏'], ['⍊'], ['⍘'], ['ツ'], ['益'], ['╭∩╮'], ['Ĺ̯'], ['◡'],
          [' ͜つ']]

EARS = [['q', 'p'], ['ʢ', 'ʡ'], ['⸮', '?'], ['ʕ', 'ʔ'], ['ᖗ', 'ᖘ'], ['ᕦ', 'ᕥ'],
        ['ᕦ(', ')ᕥ'], ['ᕙ(', ')ᕗ'], ['ᘳ', 'ᘰ'], ['ᕮ', 'ᕭ'], ['ᕳ', 'ᕲ'],
        ['(', ')'], ['[', ']'], ['¯\\_', '_/¯'], ['୧', '୨'], ['୨', '୧'],
        ['⤜(', ')⤏'], ['☞', '☞'], ['ᑫ', 'ᑷ'], ['ᑴ', 'ᑷ'], ['ヽ(', ')ﾉ'],
        ['\\(', ')/'], ['乁(', ')ㄏ'], ['└[', ']┘'], ['(づ', ')づ'], ['(ง', ')ง'],
        ['⎝', '⎠'], ['ლ(', 'ლ)'], ['ᕕ(', ')ᕗ'], ['(∩', ')⊃━☆ﾟ.*']]

TOSS = ("Heads", "Tails")

EIGHTBALL = [
    "🟢 As I see it, yes.", "🟡 Ask again later.", "🟡 Better not tell you now.",
    "🟡 Cannot predict now.", "🟡 Concentrate and ask again.",
    "🟡 Don’t count on it.", "🟢 It is certain.", "🟢 It is decidedly so.",
    "🟢 Most likely.", "🔴 My reply is no.", "🔴 My sources say no.",
    "🔴 Outlook not so good.", "🟢 Outlook good.", "🟡 Reply hazy, try again.",
    "🟢 Signs point to yes.", "🔴 Very doubtful.", "🟢 Without a doubt.", "🟢 Yes.",
    "🟢 Yes – definitely.", "🟢 You may rely on it."
]

DECIDE = ("Yes.", "No.", "Maybe.")

TABLE = ("(╯°□°）╯彡 ┻━┻", "I ran out of tables, will order more.",
         "Go do some work instead of flippin tables.")
